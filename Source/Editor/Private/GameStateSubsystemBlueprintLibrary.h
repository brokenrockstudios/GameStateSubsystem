...
public:
	...
	// UFUNCTION(BlueprintPure, Category = "GameState Subsystems", meta = (WorldContext = "ContextObject", BlueprintInternalUseOnly = "true"))
	// template <typename T>
	// static GAMESTATESUBSYSTEMED_API T* GetGameStateSubsystem(UObject* ContextObject, TSubclassOf<UGameStateSubsystem> Class)
	// {
	// 	return Cast<T>(GetGameStateSubsystem(ContextObject, Class));
	// }
